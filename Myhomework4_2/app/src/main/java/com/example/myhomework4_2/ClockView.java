...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        float hourX = centerX + (float)(radius * HOUR_NEEDLE_LENGTH_RATIO *  Math.cos(hourRadian));
        float hourY = centerY + (float)(radius * HOUR_NEEDLE_LENGTH_RATIO *  Math.sin(hourRadian));
        float minuteX = centerX + (float)(radius * MINUTE_NEEDLE_LENGTH_RATIO *  Math.cos(minuteRadian));
        float minuteY = centerY + (float)(radius * MINUTE_NEEDLE_LENGTH_RATIO *  Math.sin(minuteRadian));
        float secondX = centerX + (float)(radius * SECOND_NEEDLE_LENGTH_RATIO *  Math.cos(secondRadian));
        float secondY = centerY + (float)(radius * SECOND_NEEDLE_LENGTH_RATIO *  Math.sin(secondRadian));
        needlePaint.setAlpha(HIGHLIGHT_UNIT_ALPHA);
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,hourX,hourY,needlePaint);

        needlePaint.setAlpha(HIGHLIGHT_UNIT_ALPHA);
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,minuteX,minuteY,needlePaint);

        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        needlePaint.setAlpha(NORMAL_UNIT_ALPHA);
        canvas.drawLine(centerX,centerY,secondX,secondY,needlePaint);
        /**
         * 思路：
    ...
    private void drawTimeNumbers(Canvas canvas) {
        for(int i=1;i<=12;i++){
            double rad = 2 * Math.PI / 12 * (i+9);
            float numX=(float) (centerX - 30 + (radius * 0.82) * Math.cos(rad));
            float numY=(float) (centerY + 30 + (radius * 0.82) * Math.sin(rad));
            canvas.drawText(""+i,numX,numY,numberPaint);
        }
